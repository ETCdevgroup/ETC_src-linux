...
uint256 hashGenesisBlock("0xcc8f9284d551cf8a0156169e318cb0a6504462818e4c017a230005404ec3af84");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EternalCoin: starting difficulty is 1 / 2^12
...
{
    // EternalCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EternalCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EternalCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // EternalCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcc8f9284d551cf8a0156169e318cb0a6504462818e4c017a230005404ec3af84");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1396592432;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084926171;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EternalCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EternalCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running EternalCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EternalCoinMiner terminated\n");
        throw;
    }
}
